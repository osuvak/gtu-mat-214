...
()
{
  ...
  interval.left.fval  = this->call(interval.left.val);
  interval.left.sign  = signum(interval.left.fval);
  
  interval.right.val  = limRight_;
  interval.right.fval = this->call(interval.right.val);
  interval.right.sign = signum(interval.right.fval);

  cout << endl
       << "Initial Left    Value      : " << interval.left.val   << endl
       << "        Left  F Value      : " << interval.left.fval  << endl
       << "        Left  F Value Sign : " << interval.left.sign  << endl
       << "Initial Right   Value      : " << interval.right.val  << endl
       << "        Right F Value      : " << interval.right.fval << endl
       << "        Right F Value Sign : " << interval.right.sign << endl
       << endl;
  ...
  while ( llist_.size() < maxIter_ )
  {
    ...
    interval.mid.fval = this->call( interval.mid.val );
