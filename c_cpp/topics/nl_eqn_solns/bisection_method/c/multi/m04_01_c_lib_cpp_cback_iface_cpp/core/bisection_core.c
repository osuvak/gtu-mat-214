...
)
{
  ...
  interval.left.fval  = opt.fptr( interval.left.val  , opt.userdata );
  interval.left.sign  = signum(interval.left.fval);
  
  interval.right.val  = opt.right;
  interval.right.fval = opt.fptr( interval.right.val , opt.userdata );
  ...
  while ( size(soln) < opt.maxIter )
  {
    ...
    interval.mid.fval = opt.fptr( interval.mid.val , opt.userdata );
